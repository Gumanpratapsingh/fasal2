...
// router.get("/get-public-id/:playlistId", async (req, res) => {
//     try {
//         const playlist = await List.findById(req.params.playlistId);
//         if (!playlist) {
//             return res.status(404).json({ message: 'Playlist not found' });
...
//     } catch (error) {
//         res.status(500).json({ message: 'Error fetching public ID' });
//     }
// });
...
router.get("/get-public-id-by-name/:playlistName", async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error fetching public ID by name' });
    }
});
